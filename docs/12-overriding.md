...
When a subclass defines an instance method with the same _method descriptor_ as an instance method in the parent class[^2], we say that the instance method in the subclass _overrides_ the instance method in the parent class.  In the example above, `Circle::toString` has overridden `Object::toString`.
...
What if a subclass defines a class method with the same method descriptor as a class method in the parent class?  This is not overriding but simply hiding.  The class method in the parent class can still be invoked by using the class name.  Any other combination will be compilation error.

|   | Same Descriptor as Parent Instance Method | Same Descriptor as Parent Class Method |
|---|---|---|
| **Child Instance Method** | Overrides | Compilation Error |
| **Child Class Method** | Compilation Error | Hides |
