...
- Since we want to compare two `Pair` instances, we made `Pair` implement the `Comparable` interface too, passing in `Pair<S,T>` as the type argument to `Comparable`.
