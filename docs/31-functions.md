...
You can see that there is much boilerplate code in the two functions above that we can remove.

- The target variable has a compile-time type of a functional interface `Transformer`.  It has no constructor.  As such, we do not have to write `new Transformer<>() { .. }`.
- Since a functional interface only have one abstract method, we can infer that we are overriding `public Integer transform(..) { .. }`.
- As a good practice, we have to add `@Override` when overriding, this can also be inferred to be needed since we cannot instantiate the anonymous class otherwise.

What remain after we eliminate the obvious boilerplate code are

1. the parameter `Integer x`, and
2. the body of `transform`, which is `{ return x * x; }`.
...
Then `Fn2<.., .., ..> fn = B::bar;` can only be interpreted as `(x, y) -> B.bar(x, y)`.  In fact, we can further infer that the type should be `Fn2<Integer, Integer, Integer>`.  On the other hand `Fn2<.., .., ..> fn = C::bar;` can only be interpreted as `(c, x) -> c.bar(x)` with the type of `Fn2<C, Integer, Integer>`.  We cannot interpret `C::bar` on its own without knowing the target variable.
