...
| Move page-by-page | Anywhere | ++control+f++ | ++control+b++ | |
| Move block-by-block | Anywhere | ++open-brace++ | ++close-brace++ |  A block (or paragraph) is defined as consucutive non-empty lines.  This is useful for instance to navigate method-by-method, provided you have blank lines between two methods |
...
| Beginning/End of the file | Anywhere | ++shift+g++ | ++g++ ++g++ | Useful for jump to the end of the file (e.g., to append a new method) or to the beginning of the file (e.g., to add `import` statements)
...
!!! Tips "&lt;count&gt;-&lt;movement&gt;" 
    Instead of repeating the commands above $k$ times, you can issue the command $k$ followed by the movement.  For instance, ++6++ ++w++ would move forward by six words.  ++4++ ++control+f++ would move forward by 4 pages.
...
- Place the cursor under the identifier `Event`, then type `gf` (go-to file).  Vim will load the file `Event.java`.  After you are done, to go back to the previous file, use ++control+"^"++ (++"^"++ refers to go-back UP).
...
Use ++control+w++ ++control+w++ to navigate between windows.
...
int j = 0;
```
which does not compile.  We should, of course, declare and initialize `j` first before using it.  Placing the cursor on the line `i = 2 * j`, we can perform the two steps above. (i) Type ++d++ ++d++ to delete the line (essentially move it into the clipboard).  The cursor will move to the line `int j = 0`.  (ii) Type ++p++ to paste it after the current line.

++d++ ++d++ ++p++ essentially swaps two lines.
...
!!! Tip "Reformatting After Pasting"
    ...
    After pasting a block of code, the indentation of the pasted code might be inconsistent.  Make it a habit to use `gg=G` (i.e., ++g++ ++g++ ++"="++ ++shift+g++) to reindent your code after pasting.
...
One option is to use ++c++ ++w++ twice.  But we could also use the substitute command, like so.
...
It is often useful to split your Vim window to open up a terminal, by the `:terminal` command.  From the terminal, you can run `javac` to compile and `java` to test.  You can either ++control+w++ ++control+w++ to switch back to edit your code or ++control+d++ to close the terminal.
...
++u++ undoes the last action.  ++control+r++ redoes the action. 
