...
```Java title="Declaration and Initialization of a Point"
Point p;
p = new Point(1, 2);
```
...
[^1]: It can also behave like a function/method in the sense that it can be invoked (e.g., `this(..)`).  In this case, the keyword `this` represents the constructor of the current class.  We will illustrate more of this on the topic of overloading.
...
```Java title="Declaration and Initialization of a Circle"
Circle c;
...
```Java title="An Example of Aliasing"
Circle c;
Point center;
double radius;
radius = 8;
...
Since these are local variables that have just been declared but not yet assigned, they are _uninitialized_ (shown with ∅ in the diagrams).  Recall from the [Constructor](#constructor) section above that uninitialized variables cannot be used and have no value, not even `null`.
...
```Java title="Point with distanceTo"
class Point {
  private double x;
  private double y;
