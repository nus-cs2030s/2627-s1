...
It would be useful if there is a monad that allows us to perform the tasks concurrently.  [`java.util.concurrent.CompletableFuture`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/CompletableFuture.html) is such monad.  Here is an example of how to use it:
...
Let's now examine the `CompletableFuture` monad in more detail.  A key property of `CompletableFuture` is whether the value it promises is ready (i.e., whether the tasks that it encapsulates have been _completed_ or not).
...
Only one of the two parameters is meaningful.  If the exception is not `null`, this means that an exception has been thrown and we can use the exception (i.e., second parameter).  Otherwise, the exception is `null`[^3] and we can use the value (i.e., first parameter). 
